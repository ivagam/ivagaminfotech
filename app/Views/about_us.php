<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ivagam</title>
    <link rel="icon" href="<?php echo WEB_DIR ?>images/favicon1.png" sizes="16x16" />
<link rel="stylesheet" href="<?php echo WEB_DIR ?>css/bootstrap.min.css">
<link rel="stylesheet" href="<?php echo WEB_DIR ?>css/font-awesome.css">
<link rel="stylesheet" href="<?php echo WEB_DIR ?>css/owl.carousel.min.css">
<link rel="stylesheet" href="<?php echo WEB_DIR ?>css/owl.theme.default.min.css">
<link rel="stylesheet" href="<?php echo WEB_DIR ?>css/style.css">

</head>
<body>
    <?= view('header') ?>
    <main>
    <section class="inner-banner-section section-padding" style="background-image: url('<?php echo WEB_DIR ?>images/about.jpg');">
    <div class="container">
                <div class="row justify-content-center">
                    <div class="col-auto">
                        <div class="banner-caption">
                            <div class="title">
                                <h1>About Us</h1>
                            </div>
                            <div class="breadcrumb justify-content-center">
                                <a href="#">Home</a> / <span>About Us</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!--  -->
        <section class="about-section section-padding">
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-lg-6 order-lg-1 order-2 pe-lg-5">
                        <div class="title mb-lg-5 mb-4">
                            <h5>About Us</h5>
                            <h2 class="">About Ivagam Infotech Private Limited</h2>
                        </div>
                        <p>
                        Ivagam Infotech Private Limited has been a prominent player in the software development industry for the past eight years, driven by the leadership of directors P. Murugan and R. Pasupathi. Based in the vibrant town of Tiruvannamalai, Tamil Nadu, the company has established itself as a trusted partner for businesses seeking innovative and reliable technology solutions.
                        </p>
                        <p>
                        Company Overview Founded with a vision to provide comprehensive software services, Ivagam Infotech has grown steadily, employing a talented team of 10 professionals who bring diverse technical expertise to the table. Over the years, the company has delivered successful projects in web, mobile, and desktop application development, earning a reputation for quality and customer satisfaction.
  
</p>
                        <a href="#" class="btn_ primaary">Read more</a>
                    </div>
                    <div class="col-lg-6 mb-lg-0 mb-4 order-lg-2 order-1">
                    <img src="<?php echo WEB_DIR ?>images/ab1.png" alt="" class="img-fluid">
                    </div>
                </div>

                <div class="row">
                            <div class="item col-lg-6">
                                <div class="testimonial-box">
                                    <div class="top d-flex align-items-center">
                                        <div class="avator">
                                            <img src="<?php echo WEB_DIR?>images/ivagam-infotech-mission.jpg" alt="" class="object-cover">
                                        </div>
                                        <div class="name">
                                            <h5>Mission</h5>
                                        </div>
                                    </div>
                                    <div class="bottom">
                                    At Ivagam Infotech Private Limited, our mission is to empower businesses with innovative, reliable, and custom-built software solutions. We are committed to delivering top-quality services in web, mobile, and desktop application development, leveraging the expertise of our dedicated team to exceed client expectations and foster growth. Our focus is on building long-term partnerships through exceptional service, technical excellence, and a client-centric approach.
                                    </div>
                                </div>
                            </div>
                            <div class="item col-lg-6">
                                <div class="testimonial-box">
                                    <div class="top d-flex align-items-center">
                                        <div class="avator">
                                            <img src="<?php echo WEB_DIR?>images/ivagam-infotech-vision.jpg" alt="" class="object-cover">
                                        </div>
                                        <div class="name">
                                            <h5>Vision</h5>
                                        </div>
                                    </div>
                                    <div class="bottom">
                                    Our vision is to be a leader in the global tech industry, recognized for transforming ideas into impactful software that drives progress and innovation. We aspire to create a dynamic, technology-driven future where businesses of all sizes can harness the power of digital solutions to achieve their goals. By continuously evolving and embracing cutting-edge technologies, we aim to set the standard for software development and contribute positively to the tech ecosystem.
                                    </div>
                                </div>
                            </div>
                           
                </div>
            </div>
        </section>
        <!--  -->
        <section class="working-process-section section-padding bgtc glowtc">
            <div class="container">
                <div class="row">
                    <div class="col-12   text-center ">
                        <div class="title mb-lg-5 mb-4">
                            <h5>Working Process</h5>
                            <h2 class="">Our Working Process</h2>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-3 col-md-6 mb-lg-0 mb-4">
                        <div class="process-box">
                            <div class="icon">
                                <div class="number">1</div>
                                <svg fill="#ff7800" width="40px" height="40px" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg"><path d="M22.5 15c-.822 0-1.5.678-1.5 1.5v.785c-.393.114-.77.267-1.13.465l-.313-.314c-.582-.582-1.54-.582-2.12 0-.583.58-.583 1.54 0 2.12l.313.315c-.2.36-.354.736-.47 1.13h-.78c-.822 0-1.5.678-1.5 1.5s.678 1.5 1.5 1.5h.785c.114.393.267.77.465 1.13l-.314.313c-.582.582-.582 1.54 0 2.12.58.583 1.54.583 2.12 0l.315-.313c.36.2.736.354 1.13.47v.78c0 .822.678 1.5 1.5 1.5s1.5-.678 1.5-1.5v-.785c.393-.114.77-.267 1.13-.465l.313.314c.582.582 1.54.582 2.12 0 .583-.58.583-1.54 0-2.12l-.313-.315c.2-.36.354-.736.47-1.13h.78c.822 0 1.5-.678 1.5-1.5s-.678-1.5-1.5-1.5h-.785c-.114-.393-.267-.77-.465-1.13l.314-.313c.582-.582.582-1.54 0-2.12-.58-.583-1.54-.583-2.12 0l-.315.313c-.36-.2-.736-.354-1.13-.47v-.78c0-.822-.678-1.5-1.5-1.5zm0 1c.286 0 .5.214.5.5v1c0 .328.084.508.38.588.57.115 1.112.34 1.597.66.198.132.462.106.63-.062l.543-.543c.202-.202.505-.202.707 0 .202.202.202.505 0 .707l-.543.543c-.168.167-.195.43-.064.628.32.486.544 1.028.658 1.597.08.302.344.383.592.383h1c.286 0 .5.214.5.5 0 .286-.214.5-.5.5h-1c-.394 0-.528.158-.588.38-.115.57-.34 1.112-.66 1.597-.132.198-.106.462.062.63l.543.543c.202.202.202.505 0 .707-.202.202-.505.202-.707 0l-.543-.543c-.167-.168-.43-.195-.628-.064-.486.32-1.028.544-1.597.658-.264.07-.383.34-.383.592v1c0 .286-.214.5-.5.5-.286 0-.5-.214-.5-.5v-1c0-.37-.138-.523-.38-.588-.57-.115-1.112-.34-1.597-.66-.198-.132-.462-.106-.63.062l-.543.543c-.202.202-.505.202-.707 0-.202-.202-.202-.505 0-.707l.543-.543c.168-.167.195-.43.064-.628-.32-.486-.544-1.028-.658-1.597-.05-.252-.262-.383-.592-.383h-1c-.286 0-.5-.214-.5-.5 0-.286.214-.5.5-.5h1c.445 0 .524-.143.588-.38.115-.57.34-1.112.66-1.597.132-.198.106-.462-.062-.63l-.543-.543c-.202-.202-.202-.505 0-.707.202-.202.505-.202.707 0l.543.543c.167.168.43.195.628.064.486-.32 1.028-.544 1.597-.658.402-.092.383-.406.383-.592v-1c0-.286.214-.5.5-.5zm0 4c-1.375 0-2.5 1.125-2.5 2.5s1.125 2.5 2.5 2.5 2.5-1.125 2.5-2.5-1.125-2.5-2.5-2.5zm0 1c.834 0 1.5.666 1.5 1.5s-.666 1.5-1.5 1.5-1.5-.666-1.5-1.5.666-1.5 1.5-1.5zm-10-13C10.02 8 8 10.02 8 12.5s2.02 4.5 4.5 4.5 4.5-2.02 4.5-4.5S14.98 8 12.5 8zm0 1c1.94 0 3.5 1.56 3.5 3.5S14.44 16 12.5 16 9 14.44 9 12.5 10.56 9 12.5 9zm-1-9c-.822 0-1.5.678-1.5 1.5v1.91c-.763.21-1.494.51-2.182.9L5.943 2.437c-.582-.582-1.54-.582-2.123 0L2.406 3.85c-.58.58-.58 1.54 0 2.12l1.887 1.887c-.382.677-.68 1.394-.887 2.143H1.5c-.822 0-1.5.678-1.5 1.5v2c0 .822.678 1.5 1.5 1.5h1.908c.207.757.507 1.48.893 2.164l-1.894 1.893c-.582.582-.582 1.54 0 2.123l1.414 1.414c.582.582 1.54.582 2.123 0l1.897-1.9c.68.39 1.404.69 2.16.898V23.5c0 .822.678 1.5 1.5 1.5h2c.664 0 .66-1 0-1h-2c-.286 0-.5-.214-.5-.5v-2.234c0-.234-.16-.437-.39-.49-.93-.212-1.82-.58-2.63-1.09-.196-.124-.452-.095-.617.068l-.006.01-.003.004-2.118 2.12c-.2.2-.507.2-.71 0l-1.413-1.415c-.2-.202-.2-.508 0-.71l2.123-2.12.004-.004.002-.003c.162-.165.19-.42.067-.616-.508-.81-.874-1.7-1.085-2.63-.052-.23-.255-.39-.49-.39H1.5c-.286 0-.5-.214-.5-.5v-2c0-.286.214-.5.5-.5h2.234c.234 0 .437-.16.49-.39.21-.923.574-1.806 1.077-2.61.125-.198.095-.454-.07-.62L3.113 5.265c-.202-.202-.202-.505 0-.707l1.414-1.414c.202-.202.508-.202.71 0L7.34 5.248c.165.165.42.194.62.07.814-.512 1.71-.88 2.647-1.093.228-.052.39-.255.39-.49V1.5c0-.286.214-.5.5-.5h2c.286 0 .5.214.5.5v2.234c0 .234.16.437.39.49.93.212 1.817.58 2.626 1.089.197.123.454.094.62-.07l2.1-2.1c.202-.202.505-.202.707 0l1.414 1.414c.202.202.202.505 0 .707L19.764 7.36c-.165.164-.194.42-.07.62.506.81.87 1.697 1.08 2.63.053.228.256.39.49.39H23.5c.286 0 .5.214.5.5v2c0 .668 1 .652 1 0v-2c0-.822-.678-1.5-1.5-1.5h-1.908c-.207-.756-.505-1.48-.89-2.164l1.862-1.865c.582-.58.582-1.54 0-2.12L21.15 2.436c-.58-.582-1.54-.582-2.12 0l-1.87 1.87c-.68-.39-1.405-.69-2.16-.898V1.5c0-.822-.678-1.5-1.5-1.5z"/></svg>
                            </div>
                            <div>
                                <h4 class="mb-3 text-uppercase">Choose A Service</h4>
                                <p>Begin your journey by selecting the service that best fits your business needs from our comprehensive range of offerings. Whether it’s web development, mobile app creation, or custom desktop solutions, we are here to provide the right expertise to help you reach your goals.</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 mb-lg-0 mb-4">
                        <div class="process-box">
                            <div class="top">
                                <h4 class="mb-3 text-uppercase">Define Requirements</h4>
                                <p>Once you choose a service, our team collaborates closely with you to understand your unique requirements and project vision. This step involves in-depth discussions and detailed documentation to ensure every aspect of your project is clearly defined, setting a strong foundation for successful development.</p>
                            </div>
                            <div class="icon">
                                <div class="number">2</div>
                                <svg fill="#ff7800" width="40px" height="40px" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg"><path d="M22.5 15c-.822 0-1.5.678-1.5 1.5v.785c-.393.114-.77.267-1.13.465l-.313-.314c-.582-.582-1.54-.582-2.12 0-.583.58-.583 1.54 0 2.12l.313.315c-.2.36-.354.736-.47 1.13h-.78c-.822 0-1.5.678-1.5 1.5s.678 1.5 1.5 1.5h.785c.114.393.267.77.465 1.13l-.314.313c-.582.582-.582 1.54 0 2.12.58.583 1.54.583 2.12 0l.315-.313c.36.2.736.354 1.13.47v.78c0 .822.678 1.5 1.5 1.5s1.5-.678 1.5-1.5v-.785c.393-.114.77-.267 1.13-.465l.313.314c.582.582 1.54.582 2.12 0 .583-.58.583-1.54 0-2.12l-.313-.315c.2-.36.354-.736.47-1.13h.78c.822 0 1.5-.678 1.5-1.5s-.678-1.5-1.5-1.5h-.785c-.114-.393-.267-.77-.465-1.13l.314-.313c.582-.582.582-1.54 0-2.12-.58-.583-1.54-.583-2.12 0l-.315.313c-.36-.2-.736-.354-1.13-.47v-.78c0-.822-.678-1.5-1.5-1.5zm0 1c.286 0 .5.214.5.5v1c0 .328.084.508.38.588.57.115 1.112.34 1.597.66.198.132.462.106.63-.062l.543-.543c.202-.202.505-.202.707 0 .202.202.202.505 0 .707l-.543.543c-.168.167-.195.43-.064.628.32.486.544 1.028.658 1.597.08.302.344.383.592.383h1c.286 0 .5.214.5.5 0 .286-.214.5-.5.5h-1c-.394 0-.528.158-.588.38-.115.57-.34 1.112-.66 1.597-.132.198-.106.462.062.63l.543.543c.202.202.202.505 0 .707-.202.202-.505.202-.707 0l-.543-.543c-.167-.168-.43-.195-.628-.064-.486.32-1.028.544-1.597.658-.264.07-.383.34-.383.592v1c0 .286-.214.5-.5.5-.286 0-.5-.214-.5-.5v-1c0-.37-.138-.523-.38-.588-.57-.115-1.112-.34-1.597-.66-.198-.132-.462-.106-.63.062l-.543.543c-.202.202-.505.202-.707 0-.202-.202-.202-.505 0-.707l.543-.543c.168-.167.195-.43.064-.628-.32-.486-.544-1.028-.658-1.597-.05-.252-.262-.383-.592-.383h-1c-.286 0-.5-.214-.5-.5 0-.286.214-.5.5-.5h1c.445 0 .524-.143.588-.38.115-.57.34-1.112.66-1.597.132-.198.106-.462-.062-.63l-.543-.543c-.202-.202-.202-.505 0-.707.202-.202.505-.202.707 0l.543.543c.167.168.43.195.628.064.486-.32 1.028-.544 1.597-.658.402-.092.383-.406.383-.592v-1c0-.286.214-.5.5-.5zm0 4c-1.375 0-2.5 1.125-2.5 2.5s1.125 2.5 2.5 2.5 2.5-1.125 2.5-2.5-1.125-2.5-2.5-2.5zm0 1c.834 0 1.5.666 1.5 1.5s-.666 1.5-1.5 1.5-1.5-.666-1.5-1.5.666-1.5 1.5-1.5zm-10-13C10.02 8 8 10.02 8 12.5s2.02 4.5 4.5 4.5 4.5-2.02 4.5-4.5S14.98 8 12.5 8zm0 1c1.94 0 3.5 1.56 3.5 3.5S14.44 16 12.5 16 9 14.44 9 12.5 10.56 9 12.5 9zm-1-9c-.822 0-1.5.678-1.5 1.5v1.91c-.763.21-1.494.51-2.182.9L5.943 2.437c-.582-.582-1.54-.582-2.123 0L2.406 3.85c-.58.58-.58 1.54 0 2.12l1.887 1.887c-.382.677-.68 1.394-.887 2.143H1.5c-.822 0-1.5.678-1.5 1.5v2c0 .822.678 1.5 1.5 1.5h1.908c.207.757.507 1.48.893 2.164l-1.894 1.893c-.582.582-.582 1.54 0 2.123l1.414 1.414c.582.582 1.54.582 2.123 0l1.897-1.9c.68.39 1.404.69 2.16.898V23.5c0 .822.678 1.5 1.5 1.5h2c.664 0 .66-1 0-1h-2c-.286 0-.5-.214-.5-.5v-2.234c0-.234-.16-.437-.39-.49-.93-.212-1.82-.58-2.63-1.09-.196-.124-.452-.095-.617.068l-.006.01-.003.004-2.118 2.12c-.2.2-.507.2-.71 0l-1.413-1.415c-.2-.202-.2-.508 0-.71l2.123-2.12.004-.004.002-.003c.162-.165.19-.42.067-.616-.508-.81-.874-1.7-1.085-2.63-.052-.23-.255-.39-.49-.39H1.5c-.286 0-.5-.214-.5-.5v-2c0-.286.214-.5.5-.5h2.234c.234 0 .437-.16.49-.39.21-.923.574-1.806 1.077-2.61.125-.198.095-.454-.07-.62L3.113 5.265c-.202-.202-.202-.505 0-.707l1.414-1.414c.202-.202.508-.202.71 0L7.34 5.248c.165.165.42.194.62.07.814-.512 1.71-.88 2.647-1.093.228-.052.39-.255.39-.49V1.5c0-.286.214-.5.5-.5h2c.286 0 .5.214.5.5v2.234c0 .234.16.437.39.49.93.212 1.817.58 2.626 1.089.197.123.454.094.62-.07l2.1-2.1c.202-.202.505-.202.707 0l1.414 1.414c.202.202.202.505 0 .707L19.764 7.36c-.165.164-.194.42-.07.62.506.81.87 1.697 1.08 2.63.053.228.256.39.49.39H23.5c.286 0 .5.214.5.5v2c0 .668 1 .652 1 0v-2c0-.822-.678-1.5-1.5-1.5h-1.908c-.207-.756-.505-1.48-.89-2.164l1.862-1.865c.582-.58.582-1.54 0-2.12L21.15 2.436c-.58-.582-1.54-.582-2.12 0l-1.87 1.87c-.68-.39-1.405-.69-2.16-.898V1.5c0-.822-.678-1.5-1.5-1.5z"/></svg>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 mb-lg-0 mb-4">
                        <div class="process-box">
                            <div class="icon">
                                <div class="number">3</div>
                                <svg fill="#ff7800" width="40px" height="40px" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg"><path d="M22.5 15c-.822 0-1.5.678-1.5 1.5v.785c-.393.114-.77.267-1.13.465l-.313-.314c-.582-.582-1.54-.582-2.12 0-.583.58-.583 1.54 0 2.12l.313.315c-.2.36-.354.736-.47 1.13h-.78c-.822 0-1.5.678-1.5 1.5s.678 1.5 1.5 1.5h.785c.114.393.267.77.465 1.13l-.314.313c-.582.582-.582 1.54 0 2.12.58.583 1.54.583 2.12 0l.315-.313c.36.2.736.354 1.13.47v.78c0 .822.678 1.5 1.5 1.5s1.5-.678 1.5-1.5v-.785c.393-.114.77-.267 1.13-.465l.313.314c.582.582 1.54.582 2.12 0 .583-.58.583-1.54 0-2.12l-.313-.315c.2-.36.354-.736.47-1.13h.78c.822 0 1.5-.678 1.5-1.5s-.678-1.5-1.5-1.5h-.785c-.114-.393-.267-.77-.465-1.13l.314-.313c.582-.582.582-1.54 0-2.12-.58-.583-1.54-.583-2.12 0l-.315.313c-.36-.2-.736-.354-1.13-.47v-.78c0-.822-.678-1.5-1.5-1.5zm0 1c.286 0 .5.214.5.5v1c0 .328.084.508.38.588.57.115 1.112.34 1.597.66.198.132.462.106.63-.062l.543-.543c.202-.202.505-.202.707 0 .202.202.202.505 0 .707l-.543.543c-.168.167-.195.43-.064.628.32.486.544 1.028.658 1.597.08.302.344.383.592.383h1c.286 0 .5.214.5.5 0 .286-.214.5-.5.5h-1c-.394 0-.528.158-.588.38-.115.57-.34 1.112-.66 1.597-.132.198-.106.462.062.63l.543.543c.202.202.202.505 0 .707-.202.202-.505.202-.707 0l-.543-.543c-.167-.168-.43-.195-.628-.064-.486.32-1.028.544-1.597.658-.264.07-.383.34-.383.592v1c0 .286-.214.5-.5.5-.286 0-.5-.214-.5-.5v-1c0-.37-.138-.523-.38-.588-.57-.115-1.112-.34-1.597-.66-.198-.132-.462-.106-.63.062l-.543.543c-.202.202-.505.202-.707 0-.202-.202-.202-.505 0-.707l.543-.543c.168-.167.195-.43.064-.628-.32-.486-.544-1.028-.658-1.597-.05-.252-.262-.383-.592-.383h-1c-.286 0-.5-.214-.5-.5 0-.286.214-.5.5-.5h1c.445 0 .524-.143.588-.38.115-.57.34-1.112.66-1.597.132-.198.106-.462-.062-.63l-.543-.543c-.202-.202-.202-.505 0-.707.202-.202.505-.202.707 0l.543.543c.167.168.43.195.628.064.486-.32 1.028-.544 1.597-.658.402-.092.383-.406.383-.592v-1c0-.286.214-.5.5-.5zm0 4c-1.375 0-2.5 1.125-2.5 2.5s1.125 2.5 2.5 2.5 2.5-1.125 2.5-2.5-1.125-2.5-2.5-2.5zm0 1c.834 0 1.5.666 1.5 1.5s-.666 1.5-1.5 1.5-1.5-.666-1.5-1.5.666-1.5 1.5-1.5zm-10-13C10.02 8 8 10.02 8 12.5s2.02 4.5 4.5 4.5 4.5-2.02 4.5-4.5S14.98 8 12.5 8zm0 1c1.94 0 3.5 1.56 3.5 3.5S14.44 16 12.5 16 9 14.44 9 12.5 10.56 9 12.5 9zm-1-9c-.822 0-1.5.678-1.5 1.5v1.91c-.763.21-1.494.51-2.182.9L5.943 2.437c-.582-.582-1.54-.582-2.123 0L2.406 3.85c-.58.58-.58 1.54 0 2.12l1.887 1.887c-.382.677-.68 1.394-.887 2.143H1.5c-.822 0-1.5.678-1.5 1.5v2c0 .822.678 1.5 1.5 1.5h1.908c.207.757.507 1.48.893 2.164l-1.894 1.893c-.582.582-.582 1.54 0 2.123l1.414 1.414c.582.582 1.54.582 2.123 0l1.897-1.9c.68.39 1.404.69 2.16.898V23.5c0 .822.678 1.5 1.5 1.5h2c.664 0 .66-1 0-1h-2c-.286 0-.5-.214-.5-.5v-2.234c0-.234-.16-.437-.39-.49-.93-.212-1.82-.58-2.63-1.09-.196-.124-.452-.095-.617.068l-.006.01-.003.004-2.118 2.12c-.2.2-.507.2-.71 0l-1.413-1.415c-.2-.202-.2-.508 0-.71l2.123-2.12.004-.004.002-.003c.162-.165.19-.42.067-.616-.508-.81-.874-1.7-1.085-2.63-.052-.23-.255-.39-.49-.39H1.5c-.286 0-.5-.214-.5-.5v-2c0-.286.214-.5.5-.5h2.234c.234 0 .437-.16.49-.39.21-.923.574-1.806 1.077-2.61.125-.198.095-.454-.07-.62L3.113 5.265c-.202-.202-.202-.505 0-.707l1.414-1.414c.202-.202.508-.202.71 0L7.34 5.248c.165.165.42.194.62.07.814-.512 1.71-.88 2.647-1.093.228-.052.39-.255.39-.49V1.5c0-.286.214-.5.5-.5h2c.286 0 .5.214.5.5v2.234c0 .234.16.437.39.49.93.212 1.817.58 2.626 1.089.197.123.454.094.62-.07l2.1-2.1c.202-.202.505-.202.707 0l1.414 1.414c.202.202.202.505 0 .707L19.764 7.36c-.165.164-.194.42-.07.62.506.81.87 1.697 1.08 2.63.053.228.256.39.49.39H23.5c.286 0 .5.214.5.5v2c0 .668 1 .652 1 0v-2c0-.822-.678-1.5-1.5-1.5h-1.908c-.207-.756-.505-1.48-.89-2.164l1.862-1.865c.582-.58.582-1.54 0-2.12L21.15 2.436c-.58-.582-1.54-.582-2.12 0l-1.87 1.87c-.68-.39-1.405-.69-2.16-.898V1.5c0-.822-.678-1.5-1.5-1.5z"/></svg>
                            </div>
                            <h4 class="mb-3 text-uppercase">Request A Meeting</h4>
                            <p>We believe that effective communication is key to a successful project. Schedule a meeting with our experts to go over the project details, clarify any questions, and discuss timelines and deliverables. This collaborative session ensures alignment between your expectations and our development roadmap.</p>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 mb-lg-0 mb-4">
                        <div class="process-box">
                            
                            <h4 class="mb-3 text-uppercase">Final Solution</h4>
                            <p>After thorough planning and execution, we present the final solution tailored to your specifications. Our rigorous testing and quality assurance processes ensure a product that meets the highest standards of performance, functionality, and user experience. We deliver your solution on time, ready to make an impact on your business operations.</p>
                            <div class="icon">
                                <div class="number">4</div>
                                <svg fill="#ff7800" width="40px" height="40px" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg"><path d="M22.5 15c-.822 0-1.5.678-1.5 1.5v.785c-.393.114-.77.267-1.13.465l-.313-.314c-.582-.582-1.54-.582-2.12 0-.583.58-.583 1.54 0 2.12l.313.315c-.2.36-.354.736-.47 1.13h-.78c-.822 0-1.5.678-1.5 1.5s.678 1.5 1.5 1.5h.785c.114.393.267.77.465 1.13l-.314.313c-.582.582-.582 1.54 0 2.12.58.583 1.54.583 2.12 0l.315-.313c.36.2.736.354 1.13.47v.78c0 .822.678 1.5 1.5 1.5s1.5-.678 1.5-1.5v-.785c.393-.114.77-.267 1.13-.465l.313.314c.582.582 1.54.582 2.12 0 .583-.58.583-1.54 0-2.12l-.313-.315c.2-.36.354-.736.47-1.13h.78c.822 0 1.5-.678 1.5-1.5s-.678-1.5-1.5-1.5h-.785c-.114-.393-.267-.77-.465-1.13l.314-.313c.582-.582.582-1.54 0-2.12-.58-.583-1.54-.583-2.12 0l-.315.313c-.36-.2-.736-.354-1.13-.47v-.78c0-.822-.678-1.5-1.5-1.5zm0 1c.286 0 .5.214.5.5v1c0 .328.084.508.38.588.57.115 1.112.34 1.597.66.198.132.462.106.63-.062l.543-.543c.202-.202.505-.202.707 0 .202.202.202.505 0 .707l-.543.543c-.168.167-.195.43-.064.628.32.486.544 1.028.658 1.597.08.302.344.383.592.383h1c.286 0 .5.214.5.5 0 .286-.214.5-.5.5h-1c-.394 0-.528.158-.588.38-.115.57-.34 1.112-.66 1.597-.132.198-.106.462.062.63l.543.543c.202.202.202.505 0 .707-.202.202-.505.202-.707 0l-.543-.543c-.167-.168-.43-.195-.628-.064-.486.32-1.028.544-1.597.658-.264.07-.383.34-.383.592v1c0 .286-.214.5-.5.5-.286 0-.5-.214-.5-.5v-1c0-.37-.138-.523-.38-.588-.57-.115-1.112-.34-1.597-.66-.198-.132-.462-.106-.63.062l-.543.543c-.202.202-.505.202-.707 0-.202-.202-.202-.505 0-.707l.543-.543c.168-.167.195-.43.064-.628-.32-.486-.544-1.028-.658-1.597-.05-.252-.262-.383-.592-.383h-1c-.286 0-.5-.214-.5-.5 0-.286.214-.5.5-.5h1c.445 0 .524-.143.588-.38.115-.57.34-1.112.66-1.597.132-.198.106-.462-.062-.63l-.543-.543c-.202-.202-.202-.505 0-.707.202-.202.505-.202.707 0l.543.543c.167.168.43.195.628.064.486-.32 1.028-.544 1.597-.658.402-.092.383-.406.383-.592v-1c0-.286.214-.5.5-.5zm0 4c-1.375 0-2.5 1.125-2.5 2.5s1.125 2.5 2.5 2.5 2.5-1.125 2.5-2.5-1.125-2.5-2.5-2.5zm0 1c.834 0 1.5.666 1.5 1.5s-.666 1.5-1.5 1.5-1.5-.666-1.5-1.5.666-1.5 1.5-1.5zm-10-13C10.02 8 8 10.02 8 12.5s2.02 4.5 4.5 4.5 4.5-2.02 4.5-4.5S14.98 8 12.5 8zm0 1c1.94 0 3.5 1.56 3.5 3.5S14.44 16 12.5 16 9 14.44 9 12.5 10.56 9 12.5 9zm-1-9c-.822 0-1.5.678-1.5 1.5v1.91c-.763.21-1.494.51-2.182.9L5.943 2.437c-.582-.582-1.54-.582-2.123 0L2.406 3.85c-.58.58-.58 1.54 0 2.12l1.887 1.887c-.382.677-.68 1.394-.887 2.143H1.5c-.822 0-1.5.678-1.5 1.5v2c0 .822.678 1.5 1.5 1.5h1.908c.207.757.507 1.48.893 2.164l-1.894 1.893c-.582.582-.582 1.54 0 2.123l1.414 1.414c.582.582 1.54.582 2.123 0l1.897-1.9c.68.39 1.404.69 2.16.898V23.5c0 .822.678 1.5 1.5 1.5h2c.664 0 .66-1 0-1h-2c-.286 0-.5-.214-.5-.5v-2.234c0-.234-.16-.437-.39-.49-.93-.212-1.82-.58-2.63-1.09-.196-.124-.452-.095-.617.068l-.006.01-.003.004-2.118 2.12c-.2.2-.507.2-.71 0l-1.413-1.415c-.2-.202-.2-.508 0-.71l2.123-2.12.004-.004.002-.003c.162-.165.19-.42.067-.616-.508-.81-.874-1.7-1.085-2.63-.052-.23-.255-.39-.49-.39H1.5c-.286 0-.5-.214-.5-.5v-2c0-.286.214-.5.5-.5h2.234c.234 0 .437-.16.49-.39.21-.923.574-1.806 1.077-2.61.125-.198.095-.454-.07-.62L3.113 5.265c-.202-.202-.202-.505 0-.707l1.414-1.414c.202-.202.508-.202.71 0L7.34 5.248c.165.165.42.194.62.07.814-.512 1.71-.88 2.647-1.093.228-.052.39-.255.39-.49V1.5c0-.286.214-.5.5-.5h2c.286 0 .5.214.5.5v2.234c0 .234.16.437.39.49.93.212 1.817.58 2.626 1.089.197.123.454.094.62-.07l2.1-2.1c.202-.202.505-.202.707 0l1.414 1.414c.202.202.202.505 0 .707L19.764 7.36c-.165.164-.194.42-.07.62.506.81.87 1.697 1.08 2.63.053.228.256.39.49.39H23.5c.286 0 .5.214.5.5v2c0 .668 1 .652 1 0v-2c0-.822-.678-1.5-1.5-1.5h-1.908c-.207-.756-.505-1.48-.89-2.164l1.862-1.865c.582-.58.582-1.54 0-2.12L21.15 2.436c-.58-.582-1.54-.582-2.12 0l-1.87 1.87c-.68-.39-1.405-.69-2.16-.898V1.5c0-.822-.678-1.5-1.5-1.5z"/></svg>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!--  -->
        <section class="wcu-section section-padding">
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-lg-6 order-lg-1 order-2">
                        <div class="title mb-lg-5 mb-4">
                            <h5>Why Choose Us at Ivagam Infotech Private Limited</h5>
                            <h2>Why Choose Us at Ivagam Infotech Private Limited</h2>
                        </div>
                        <ul>
                            <li><b>Proven Experience and Expertise:</b> With over eight years in the industry, Ivagam Infotech has built a reputation for delivering reliable and innovative software solutions. Our team comprises skilled professionals with extensive experience in web, mobile, and desktop application development.
                            </li>
                            <li><b>Client-Centric Approach:</b> We prioritize understanding our clients' unique needs and tailor our services to ensure their goals are met effectively. Your success is our top priority, and we are committed to providing personalized solutions that align with your vision.
                            </li>
                            <li><b>Comprehensive Services:</b> From web and mobile app development to desktop software, we offer a wide range of services under one roof. This means you can rely on us as your all-in-one tech partner for diverse development projects.
                            </li>
                            <li><b>Cutting-Edge Technology:</b> We stay at the forefront of technology by continuously adopting and integrating the latest tools and practices. This allows us to create modern, scalable, and secure software solutions that keep you ahead in the competitive landscape.
                            </li>
                            <li><b>Transparency and Communication:</b> We believe in maintaining open lines of communication with our clients throughout every stage of development. This ensures that you are informed, involved, and confident in the progress of your project.
                            </li>
                            <li>
                            <b>Timely Delivery and Support:</b> We understand the importance of deadlines and are committed to delivering projects on time without compromising quality. Our support team is also available to assist you post-deployment, ensuring seamless operation and addressing any queries or concerns.

                            </li>
                            <li><b>Dedicated Team:</b> Our team of passionate developers, designers, and strategists work collaboratively to deliver exceptional results. We foster a supportive and innovative work culture that reflects in the quality of our solutions.
                            </li>
                            
                        </ul>
                        <p>Choose Ivagam Infotech Private Limited for a trusted partner who will guide you through the digital transformation journey, providing solutions that are not only functional but drive business success.</p>
                    </div>
                    <div class="col-lg-6 order-lg-2 order-1">
                    <img src="<?php echo WEB_DIR ?>images/ab2.png" alt="" class="img-fluid">
                    </div>
                </div>
            </div>
        </section>

        
        <!--  -->
        <section class="our-team-section section-padding bgtc glowtc">
            <div class="container">
                <div class="row align-items-center  mb-lg-0 mb-4">
                    <div class="col-lg-6">
                        <div class="title mb-lg-5 mb-4">
                            <h5>Our team</h5>
                            <h2>Meet Our Professional Team Members</h2>
                        </div>
                    </div>
                    <div class="col-auto ms-lg-auto">
                        <a href="#" class="btn_ primaary">All Members</a>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12 team-carousel px-0">
                        <div class="owl-carousel">
                            <div class="item">
                                <div class="team-box">
                                    <div class="img-box">
                                    <img src="<?php echo WEB_DIR ?>images/teams/ivagam-infotech-saravanan.jpg" alt="" class="object-cover">
                                    </div>
                                    <h4><a href="#">Saravanan M</a></h4>
                                    <p>Full Stack Developer</p>
                                </div>
                            </div>
                            <div class="item">
                                <div class="team-box">
                                    <div class="img-box">
                                    <img src="<?php echo WEB_DIR ?>images/teams/ivagam-infotech-freeda.jpeg" alt="" class="object-cover">
                                    </div>
                                    <h4><a href="#">Freeda</a></h4>
                                    <p>PHP Developer</p>
                                </div>
                            </div>
                            <div class="item">
                                <div class="team-box">
                                    <div class="img-box">
                                    <img src="<?php echo WEB_DIR ?>images/teams/ivagam-infotech-gowtham.jpeg" alt="" class="object-cover">
                                    </div>
                                    <h4><a href="#">Gowtham R</a></h4>
                                    <p>PHP Developer</p>
                                </div>
                            </div>
                            <div class="item">
                                <div class="team-box">
                                    <div class="img-box">
                                        <img src="<?php echo WEB_DIR?>images/teams/ivagam-infotech-vishal.jpeg" alt="" class="object-cover">
                                    </div>
                                    <h4><a href="#">Vishal M</a></h4>
                                    <p>.Net Developer</p>
                                </div>
                            </div>
                            <div class="item">
                                <div class="team-box">
                                    <div class="img-box">
                                        <img src="<?php echo WEB_DIR?>images/teams/ivagam-infotech-pandi.jpeg" alt="" class="object-cover">
                                    </div>
                                    <h4><a href="#">Pandi P</a></h4>
                                    <p>PHP Developer</p>
                                </div>
                            </div>
                                                        
                            <div class="item">
                                <div class="team-box">
                                    <div class="img-box">
                                        <img src="<?php echo WEB_DIR?>images/teams/ivagam-infotech-prasanth.jpeg" alt="" class="object-cover">
                                    </div>
                                    <h4><a href="#">Prasanth G</a></h4>
                                    <p>.Net Developer</p>
                                </div>
                            </div>
                            
                            <div class="item">
                                <div class="team-box">
                                    <div class="img-box">
                                        <img src="<?php echo WEB_DIR?>images/teams/ivagam-infotech-vaishnavi.jpeg" alt="" class="object-cover">
                                    </div>
                                    <h4><a href="#">Vaishnavi </a></h4>
                                    <p>Java Full Stack Developer</p>
                                </div>
                            </div>

                            <div class="item">
                                <div class="team-box">
                                    <div class="img-box">
                                        <img src="<?php echo WEB_DIR?>images/teams/ivagam-infotech-parameshvaran.jpeg" alt="" class="object-cover">
                                    </div>
                                    <h4><a href="#">Parameshvaran S</a></h4>
                                    <p>.Net Developer</p>
                                </div>
                            </div>
                           
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!--  -->
        <section class="testimonial-section section-padding">
            <div class="container">
                <div class="row">
                    <div class="col-12 text-center">
                        <div class="title mb-lg-5 mb-4">
                            <h5>Tag name</h5>
                            <h2>Teastimonials</h2>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col">
                        <div class="owl-carousel">
                           <?php foreach ($testimonials as $testimonial) : ?>
                                <div class="item">
                                    <div class="testimonial-box">
                                        <div class="top d-flex align-items-center">
                                            <div class="avator">
                                                <img src="<?php echo WEB_DIR ?>images/<?php echo $testimonial['profile_image']; ?>" alt="<?php echo $testimonial['name']; ?>" class="object-cover">
                                                </div>
                                            <div class="name">
                                                <h5><?php echo $testimonial['name']; ?></h5>
                                            </div>
                                        </div>
                                        <div class="bottom">
                                            "<?php echo $testimonial['description']; ?>"
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                            
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!--  -->
    </main>
<?= view('footer') ?>

<script src="<?php echo WEB_DIR ?>js/jquery.min.js"></script>
<script src="<?php echo WEB_DIR ?>js/bootstrap.bundle.min.js"></script>
<script src="<?php echo WEB_DIR ?>js/owl.carousel.min.js"></script>
<script src="<?php echo WEB_DIR ?>js/custom.js"></script>

     
</body>
</html>